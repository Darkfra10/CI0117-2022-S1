...
typedef struct shared_data {
    ...
    pthread_mutex_t can_access_thread_counter;
    pthread_cond_t condvar_barrier;
} shared_data_t;
...
int create_threads(size_t thread_count) {
    ...
    if (threads) {
        ...
        if (private_data && shared_data) {
            ...
            shared_data->thread_counter = 0;
            pthread_mutex_init(&shared_data->can_access_position, NULL);
            pthread_cond_init(&shared_data->condvar_barrier, NULL);
            ...
            pthread_cond_destroy(&shared_data->condvar_barrier);
            free(shared_data);
...
void* run(void* params) {
    ...
    pthread_mutex_lock(&shared_data->can_access_thread_counter);
    shared_data->thread_counter++;
    if (shared_data->thread_counter < shared_data->thread_count) {
        pthread_cond_wait(&shared_data->condvar_barrier, 
            &shared_data->can_access_thread_counter);
    } else {
        pthread_cond_broadcast(&shared_data->condvar_barrier);
    }
    pthread_mutex_unlock(&shared_data->can_access_thread_counter);
